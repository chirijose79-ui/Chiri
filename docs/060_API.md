...
Cuando la autenticación sea correcta, la API deberá devolver una respuesta de éxito que contenga las credenciales de sesión.
...
| `refresh_token` | string      | Token utilizado para renovar la sesión             |
...
El `refresh_token` deberá utilizarse únicamente para el mecanismo de renovación de sesión.
...
El mensaje de error no deberá revelar si el usuario existe o no cuando esto pueda facilitar la enumeración de cuentas.

## 4.7.6 Usuario Inactivo o Bloqueado
...
El detalle específico del estado del usuario no deberá exponer información sensible innecesaria.
...
El cliente Android deberá almacenar los tokens utilizando mecanismos seguros definidos por la arquitectura de seguridad de Chiri Platform.
...
### Una decisión que debemos dejar marcada

Hay un punto que **todavía no está respaldado por la documentación existente**: los nombres concretos:
...
Los estoy proponiendo para cerrar el contrato, no afirmando que ya sean parte de Chiri.

**Antes de pegarlo definitivamente**, yo revisaría contigo esos nombres y, sobre todo, el formato general de respuesta:
...
porque ese formato **sí está definido actualmente en `060_API.md`**.

Si aprobamos este `4.7`, seguimos con **4.8 Refresh**, manteniendo exactamente la misma estructura.
...
Claro. Como aprobamos `4.7`, el `4.8` debe mantener exactamente el mismo nivel de detalle y usar el mismo contrato de respuesta.

**Reemplaza todo el contenido actual de `4.8 Endpoint de Renovación de Sesión` por este bloque:**
...
El mecanismo permitirá mantener una sesión activa sin solicitar nuevamente las credenciales del usuario mientras la sesión continúe siendo válida.
...
El endpoint no utilizará el access token como mecanismo principal de autenticación.

La autenticación de la solicitud de renovación se realizará mediante el refresh token.
...
| Campo           | Tipo   | Requerido | Descripción                                               |
| --------------- | ------ | --------: | --------------------------------------------------------- |
| `refresh_token` | string |        Sí | Token utilizado para solicitar la renovación de la sesión |

El refresh token deberá transmitirse únicamente mediante HTTPS.
...
Validación del Refresh Token
   ...
   |       |
   |       v
   |   Generación de nuevas credenciales
   |
   +-- Token inválido
           |
           v
...
Cuando la renovación sea correcta, la API deberá devolver una respuesta de éxito.
...
| --------------- | ----------- | -------------------------------------------------------- |
| `success`       | boolean     | Indica que la operación fue correcta                     |
| `data`          | object      | Nuevas credenciales de sesión                            |
| `access_token`  | string      | Nuevo token utilizado para acceder a recursos protegidos |
| `refresh_token` | string      | Nuevo token utilizado para futuras renovaciones          |
| `token_type`    | string      | Tipo de autenticación utilizado                          |
| `message`       | string/null | Mensaje opcional                                         |

El nuevo `access_token` deberá utilizarse posteriormente mediante:

```http
Authorization: Bearer NEW_ACCESS_TOKEN
```

El nuevo `refresh_token` deberá utilizarse para las siguientes renovaciones de sesión.

## 4.8.5 Rotación del Refresh Token

Cuando la plataforma genere un nuevo refresh token durante una renovación, el refresh token anterior deberá quedar invalidado.

El objetivo es evitar que un refresh token utilizado anteriormente pueda continuar siendo reutilizado indefinidamente.

El Backend deberá garantizar que un refresh token invalidado no pueda utilizarse nuevamente.

## 4.8.6 Refresh Token Inválido

Cuando el refresh token no sea válido, la API deberá rechazar la solicitud.
...
La API no deberá revelar información interna relacionada con la razón exacta por la que el token dejó de ser válido.

## 4.8.7 Refresh Token Expirado

Cuando el refresh token haya expirado, la renovación deberá ser rechazada.
...
El cliente deberá considerar que la sesión requiere una nueva autenticación.

## 4.8.8 Sesión Invalidada

Cuando la sesión asociada al refresh token haya sido invalidada, la renovación deberá ser rechazada.

Esto puede ocurrir, entre otros casos, cuando:

* el usuario cerró sesión.
* la sesión fue revocada.
* el acceso fue cerrado remotamente.
* el usuario dejó de estar autorizado.
* se detectó una condición de seguridad que requiere invalidar la sesión.

El cliente deberá eliminar las credenciales locales y solicitar nuevamente las credenciales del usuario.

## 4.8.9 Resultado de la Renovación
...
* utilizar HTTPS.
* no registrar refresh tokens en texto plano.
* no devolver información sensible innecesaria.
* validar el refresh token en el Backend.
* impedir el uso de refresh tokens invalidados.
* proteger las nuevas credenciales generadas.
* mantener la sesión asociada al usuario correspondiente.

El cliente Android deberá almacenar las nuevas credenciales utilizando los mecanismos seguros definidos por la arquitectura de seguridad de Chiri Platform.
...
He mantenido aquí la decisión que acabamos de aprobar para `4.7`:
...
Además, el bloque propone **rotación del refresh token**. Esa es una decisión de seguridad más concreta que la documentación actual; por tanto, al pegar este punto estamos **cerrando también esa decisión arquitectónica**. Si queremos que Chiri v1.0 utilice refresh-token rotation, este texto está listo.
