...
Durante el desarrollo local podrá utilizarse HTTP.

Cuando la API sea expuesta fuera del entorno local o de la red
interna controlada, deberá utilizar HTTPS.

La configuración de TLS corresponde a la etapa de despliegue.
...
La arquitectura actual está centrada principalmente en el dominio
...
Los endpoints de autenticación actualmente implementados son:
...
Los endpoints protegidos requieren autenticación mediante access token.

El endpoint `/auth/me` se utiliza para validar la sesión autenticada
del cliente.

El endpoint `/auth/refresh` permite obtener un nuevo access token
mediante el mecanismo de refresh token.
...
La API utilizará una estructura estándar para las respuestas de error.
...
Actualmente, los errores de autenticación utilizarán principalmente:
...
se encuentran definidos en la sección `6.7 Códigos de Error`.
