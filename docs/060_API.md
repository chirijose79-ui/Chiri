...
* `401 Unauthorized` para solicitudes no autenticadas o cuando las
  credenciales, tokens o sesiones no son válidos.

* `200 OK` para operaciones exitosas de consulta cuando corresponda.

* Los endpoints de autenticación que utilizan `POST` deberán devolver
  el código HTTP definido por su contrato específico.
...
Cuando se implemente la autorización granular, también almacenará:
...
El uso de `kid` queda establecido como parte del diseño de identificación de claves
y deberá implementarse cuando se incorpore gestión y rotación de claves.
...
* algoritmo `RS256`.
* `kid`, cuando sea utilizado por la implementación.
* `iss` válido.
* `aud` válido.
* `iat` válido.
* `exp` válido.
...
Los errores de la API deberán utilizar códigos estables, descriptivos y definidos como parte del contrato de comunicación.

Los códigos permitirán que los clientes identifiquen la categoría del error sin depender del texto descriptivo del mensaje.
...
## 6.7.1 Errores de Autenticación

Los errores relacionados con la autenticación podrán utilizar códigos como:
...
AUTH_REQUIRED
        ...
        +-- No se proporcionaron credenciales
            cuando el endpoint requiere autenticación

AUTH_INVALID_ACCESS_TOKEN
        |
        +-- Access token proporcionado pero inválido,
            expirado o asociado a una sesión no válida

AUTH_INVALID_REFRESH_TOKEN
        |
        +-- Refresh token proporcionado pero inexistente,
            inválido, expirado, revocado o asociado
            a una sesión no válida
...
La respuesta HTTP utilizada para errores de autenticación será `401 Unauthorized` cuando corresponda según el contrato del endpoint.

## 6.7.2 Errores de Autorización
