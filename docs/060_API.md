...
En despliegue público, el endpoint se expone mediante:
...
Requiere un Access Token válido mediante:

```http
Authorization: Bearer <access_token>
```

Si el token no existe, es inválido o la sesión ya no es válida, la API responderá:
...
Devuelve el resumen general de Home para el usuario autenticado.
...
| -------------------------- | ------- | ------------------------------------ |
| `user.display_name`        | string  | Nombre mostrado al usuario.          |
| `home.status`              | string  | Estado general del hogar.            |
| `quick_actions`            | array   | Acciones rápidas disponibles.        |
| `quick_actions[].id`       | string  | Identificador de la acción.          |
| `quick_actions[].enabled`  | boolean | Indica si la acción está habilitada. |
| `information.connectivity` | string  | Estado de conectividad.              |
| `information.server`       | string  | Estado del servidor.                 |
...
| HTTP  | Situación                                                    |
| ----- | ------------------------------------------------------------ |
| `401` | No autenticado, token inválido o sesión no válida.           |
| `500` | Error interno del servidor. No se exponen detalles internos. |

El endpoint no devuelve contraseñas, hashes, tokens ni información sensible de autenticación.

### Alcance v1.0

El endpoint Home proporciona únicamente información de resumen y estado general.

No implementa:

* control directo de dispositivos.
* lógica específica de Home Assistant.
