...
## 4.5.4 Claims del Access Token

El JWT deberá contener únicamente la información necesaria para identificar y validar la credencial.

Como mínimo, el diseño contempla:
...
Los claims del Access Token serán:

| Claim | Descripción |
| --- | --- |
| `sub` | Identificador del usuario autenticado |
| `sid` | Identificador de la sesión asociada |
| `jti` | Identificador único del Access Token |
| `iat` | Fecha y hora de emisión del token |
| `exp` | Fecha y hora de expiración del token |
| `iss` | Emisor del token. Será `chiri-api` |
| `aud` | Audiencia del token. Será `chiri-platform` |

Los valores anteriores forman parte del contrato de autenticación de Chiri Platform.

La implementación del servicio de autenticación deberá respetar estos nombres, tipos y significados.
...
## 4.5.6 Validación del Access Token

Para aceptar un access token, el Backend deberá comprobar como mínimo:

* existencia del encabezado `Authorization`;
* formato `Bearer`;
...
* `iss` válido;
* `aud` válido;
* firma válida mediante la clave pública correspondiente;
* vigencia del token mediante `exp`;
* validez temporal mediante `iat`;
* existencia de `sub`;
* existencia de `sid`;
* existencia de `jti`;
...
* asociado a una sesión expirada;
...
## 4.5.7 Asociación con la Sesión

Cada access token deberá contener el identificador de la sesión asociada mediante el claim:
...
La sesión será la autoridad para determinar si el acceso continúa siendo válido.

Aunque el JWT pueda validarse criptográficamente, una sesión `REVOKED` o `EXPIRED` deberá impedir el acceso a recursos protegidos.
...
## 4.8.9 Resultado de la Operación
